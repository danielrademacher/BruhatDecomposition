...
InstallGlobalFunction(  LGOStandardGensNC,
function( d, q )
...
# (cf Wreath-Products and Signed Permutation Matrices)
...
# of a matrix M in < s,v,x > under the above map by Mwr.
...
function( M )
    ...
    for r  in [ 1..d ] do
        ...
        # If there is no non-zero entry or there was already one in column j
        # ( The latter means two non-zero entries in the same column )
        if d < j or found[j]  then
            return false;
        fi;
        ...
        # If there is a second non-zero entry in row r then
        if PositionNot( M[r], zero, j ) <= d  then
            return false;
        fi;
...
InstallGlobalFunction(  BruhatDecompositionNC,
function(stdgens, g)
...
InstallGlobalFunction(  BruhatDecompositionWithTiNC,
function(stdgens, g)
